...
<!--# if expr="$theme = ubuntu804" --><!--# include virtual="/personal/chrome/mid.html" --><!--# endif -->
  ...
  <main id="content">
    <h1 class="post-title"><span>{{title_h1}}</span>{{?pangram}}<a href="{{pangram}}"><img src="pangramHumanBadge.webp" alt="{{s_pangram_alt}}" width="88" height="31"></a>{{/pangram}}</h1>
    <!-- The post's own dateline. Cross-checked against the list in
         blog.html by assetsBuild/makeMeta.py, along with the dates in
         the JSON-LD above. -->
    <p class="dateline"><time datetime="{{date}}">{{date}}</time>{{?edited}}, {{s_edited}} <time datetime="{{updated}}">{{updated}}</time>{{/edited}}</p>
