...
for my $f (@files) {
    ...
    # Gate on the TYPE, not the @id (see stamp() and githooks.txt): the site
    # @id also appears as isPartOf on the /personal/ pages, so gating on it
    # would drag every one of them to the unstaged-changes warning below.
